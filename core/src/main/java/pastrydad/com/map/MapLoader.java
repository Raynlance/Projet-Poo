...
public class MapLoader {
    ...
    public static GameMap loadMap(String tmxFilePath) {
        try {
            ...
            TilesetManager tilesetManager = new TilesetManager(tileWidth, tileHeight);
            tilesetManager.loadTileset(doc);
            gameMap.setTilesetManager(tilesetManager);
    ...
    private static List<Tile>[][] loadAllLayers(Document doc, int mapWidth, int mapHeight,
                                         Map<Integer, TileProperties> tilePropertiesMap) {
        
        @SuppressWarnings("unchecked")
        List<Tile>[][] tiles = new ArrayList[mapHeight][mapWidth];

        
        for (int y = 0; y < mapHeight; y++) {
            for (int x = 0; x < mapWidth; x++) {
                tiles[y][x] = new ArrayList<>();
            }
        }
        ...
        for (int i = 0; i < tileLayers.size(); i++) {
            Element layer = tileLayers.get(i);
            String layerName = layer.getAttribute("name");
            System.out.println("Processing layer " + (i + 1) + "/" + tileLayers.size() + ": " + layerName);
            ...
            for (int y = 0; y < mapHeight; y++) {
                for (int x = 0; x < mapWidth; x++) {
                    ...
                    if (layerTile != null && layerTile.getTileId() != 0) {
                        ...
                        tiles[y][x].add(layerTile);
                    }
                }
            }
        }

        System.out.println("All " + tileLayers.size() + " layers loaded into memory");
        return tiles;
    }
    ...
    private static List<SpawnPoint> loadSpawnPoints(Document doc, int tileSize) {
        ...
        for (int i = 0; i < objectGroupNodes.getLength(); i++) {
            Element objectGroupElement = (Element) objectGroupNodes.item(i);
            if (objectGroupElement.getAttribute("name").equals("spawn_point")) {
        ...
        if (spawnLayer == null) {
            System.out.println("No spawn_point layer found");
            return spawnPoints;
        }
